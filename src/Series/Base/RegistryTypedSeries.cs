namespace Radical.Series.Base
{
    ...
    public abstract class RegistryTypedSeries<V> : TypedSeriesBase<V> where V : IUnique
    {
        ...
        protected virtual ISeriesItem<V> GetItem(ulong key, V item)
        {
            if (key == 0)
                return null;
        ...
        protected ISeriesItem<V> createNew(ulong key, V value)
        {
            int id = count + removed;
        ...
        {
            var _item = createNew(item.Key, item.Value);
            item.Value = value;
        ...
        {
            ulong key = unique.Key(value, value.TypeKey);
        ...
        protected override int IndexOf(ulong key, V value)
        {
            if (!repeatable)
