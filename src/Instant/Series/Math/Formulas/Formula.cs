namespace Radical.Instant.Series.Math.Formulas
{
    using Set;
    using Operation;
    ...
    public abstract class Formula
    {
        ...
        public CompiledMathSet CompileMathSet(CombinedFormula m)
        {
            return InstantSeriesMathCompiler.Compile(m);
        }

        public CompiledMathSet CompileMathSet(Formula f, LeftFormula m)
        {
            CompiledMathSet mathline = InstantSeriesMathCompiler.Compile(new CombinedFormula(m, f));
            return mathline;
        }

        public abstract void Compile(ILGenerator g, InstantSeriesMathCompilerContext cc);

        public void Compute(CompiledMathSet cm)
        {
        ...
        {
            CompiledMathSet mathline = CompileMathSet(m);
            Evaluator ev = new Evaluator(mathline.Compute);
            return ev;
        }

        public Evaluator CreateEvaluator(CompiledMathSet e)
        {
        ...
        {
            CompiledMathSet mathline = CompileMathSet(f, m);
            Evaluator ev = new Evaluator(mathline.Compute);
            return ev;
        }
