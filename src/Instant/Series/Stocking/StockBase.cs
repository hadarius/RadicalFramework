...
namespace Radical.Instant.Series.Stocking
{
    ...
    public abstract unsafe class StockBase : IDisposable
    {
        ...
        {
            long bufferSize = itemSize * length;
            ...
            if (IsOwnerOfSharedMemory)
            {
                ...
                ItemType = itemType;
                ItemSize = (itemType != null) ? (int)itemSize : -1;
                FixSize = fixSize;
            }
        }
...
#pragma warning restore 0420
        ...
        {
            Extracting.Extract.CopyBlock(BufferStartPtr, (ulong)position, source, 0, (ulong)length);
        }
        ...
        {
            Extracting.Extract.CopyBlock(destination, 0, BufferStartPtr, (ulong)position, (ulong)length);
        }
