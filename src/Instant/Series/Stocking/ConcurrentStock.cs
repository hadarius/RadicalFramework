namespace Radical.Instant.Series.Stocking
{
    ...
    public unsafe class ConcurrentStock : Stock
    {
        ...
        public ConcurrentStock(string file, string name) : this(file, name, 0, 0, false)
        {
            Open();
        }

        private ConcurrentStock(
            string file,
            string name,
            int nodeCount,
            long nodeBufferSize,
            ...
            : base(
                ...
                name,
                1,
                Marshal.SizeOf(typeof(NodeHeader))
            ...
            )
        {
            if (ownsSharedMemory && nodeCount < 2)
                throw new ArgumentOutOfRangeException(
                    "nodeCount",
                    nodeCount,
                    "The node count must be a minimum of 2."
                );
#if DEBUG
        ...
        {
            DataExists = new EventWaitHandle(
                false,
                EventResetMode.AutoReset,
                Name + "_evt_dataexists"
            );
            NodeAvailable = new EventWaitHandle(
                false,
                EventResetMode.AutoReset,
                Name + "_evt_nodeavail"
            );
        ...
        private void InitialiseLinkedListNodes()
        {
            ...
            base.Write(
                nodes.SelectMany(o => new object[] { o }).ToArray(),
                0,
                nodes.Length,
                NodeOffset,
                typeof(Node)
...
#pragma warning disable 0420
                if (
                    Interlocked.CompareExchange(ref _nodeHeader->WriteStart, node->Next, blockIndex)
                    == blockIndex
                )
                    return node;
...
#pragma warning restore 0420
        ...
        public virtual int Write(byte[] source, int startIndex = 0, int timeout = 1000)
        {
            ...
            Marshal.Copy(
                source,
                startIndex,
                new IntPtr(BufferStartPtr + node->Offset),
                (int)amount
            );
        ...
        )
        {
            ...
            long count = Math.Min(
                ...
                NodeBufferSize / Marshal.SizeOf(source.GetType())
            );
...
#pragma warning disable 0420
                if (
                    Interlocked.CompareExchange(ref _nodeHeader->ReadStart, node->Next, blockIndex)
                    == blockIndex
                )
                    return node;
#pragma warning restore 0420
...
#pragma warning restore 0420
        ...
        )
        {
            ...
            int count = (int)
                Math.Min(destination.Length - startIndex, node->AmountWritten / Marshal.SizeOf(t));
